...
// ===== Validation เวลา =====
if (action === 'checkin' && timeStr > '08:31:00') {
  return json({
    success: false,
    message: 'ไม่สามารถลงเวลาเข้าปฏิบัติงานได้ เนื่องจากพ้นระยะเวลาที่กำหนด กรุณายื่นคำขอลงเวลาย้อนหลัง'
  }, 400);
}

if (action === 'checkout' && timeStr < '16:30:00') {
  return json({
    success: false,
    message: 'ไม่สามารถลงเวลาออกปฏิบัติงานได้ เนื่องจากยังไม่ถึงเวลาที่กำหนด'
  }, 400);
}
...
// ตรวจสอบว่ามีการส่งพิกัดมาหรือไม่
...
if (lat === null || lon === null) {
  return json({
    success: false,
    message: 'ไม่สามารถตรวจสอบตำแหน่งได้ กรุณาเปิด GPS และอนุญาตให้เข้าถึงตำแหน่งของอุปกรณ์'
  }, 400);
}
...
if (lat === null || lon === null) {
  return json({
    success: false,
    message: 'กรุณากำหนดพิกัดที่ตั้งให้ถูกต้อง'
  }, 400);
}
    ...
    try {
      if (existing) {
        ...
        `).bind(
          uuid, dateISO,
          timeStr, work_type || 'ปกติ', note || null,
          null, null, finalDistanceM, inRangeVal,
          ref, isoString, // ✅ ใช้ isoString ที่คำนวณไว้
