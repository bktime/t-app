...
export async function onRequest(context) {
  ...
  try {
    ...
    const { nationalId, pin } = body;

    if (!nationalId || !pin) {
      return json({ success: false, message: 'กรุณากรอกข้อมูลให้ครบถ้วน' }, 400);
    }

    // ✅ บังคับว่าต้องเป็นตัวเลข 6 หลักเท่านั้น
    if (!/^\d{6}$/.test(pin)) {
      return json({ success: false, message: 'รหัส Kiosk ต้องเป็นตัวเลข 6 หลัก' }, 400);
    }

    // 1. ค้นหา User จากเลขบัตร
    ...
    if (!user) {
      return json({ success: false, message: 'ไม่พบเลขบัตรประชาชนนี้ในระบบ' }, 400);
    }
    ...
    // 2. ตรวจสอบว่ามีรหัส Kiosk หรือยัง
    if (!user.kiosk_pin_hash) {
      return json({ success: false, message: 'บัญชีนี้ยังไม่ได้สร้างรหัสผ่านสำหรับ Kiosk' }, 400);
    }

    // 3. Hash รหัสที่กรอกมา และเทียบกับ kiosk_pin_hash
    const hashedInput = await hashPassword(pin, user.uuid);
    
    // ✅ เช็คเฉพาะรหัส Kiosk
    if (hashedInput !== user.kiosk_pin_hash) {
      return json({ success: false, message: 'รหัส Kiosk ไม่ถูกต้อง' }, 400);
    }
    ...
    const authToken  = await generateSecureToken(user.uuid, 'kiosk', env);
    ...
    `).bind(
      ...
      deviceInfo.os,
      ip,
      'kiosk', // ✅ ระบุ social_type ว่ามาจาก Kiosk
    ...
    return json({
      ...
      message: 'เข้าสู่ระบบ Kiosk สำเร็จ'
    });
...
async function generateSecureToken(userUuid, socialType, env) {
  ...
  const encoder = new TextEncoder();
  const secretKey = env.TOKEN_SECRET || 'anicca-dukkha-anatta';
